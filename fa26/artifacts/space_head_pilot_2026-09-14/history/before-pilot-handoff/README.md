...
**Documentation and planning only; do not execute experiments or watchers.**
The [meeting notes](docs/2026-09-14-MEETING-NOTES.md) record three tasks:
batch effects / contrastive learning (Brain), spaceflight prediction and gene attribution
(Minggang), and human versus mouse (owner/objective to be defined).
Use [Minggang's focused plan](docs/2026-09-14-SPACE-PREDICTION-HEAD-PLAN.md) for the
current personal workstream. Its initial frozen-model head does not depend on completing
batch correction. No training has started.
