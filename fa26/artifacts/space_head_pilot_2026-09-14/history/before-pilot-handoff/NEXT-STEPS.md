...
**Plan and document only. Do not execute jobs or start a watcher.**
The [meeting notes](docs/2026-09-14-MEETING-NOTES.md) assign batch/contrastive work to
Brain and spaceflight prediction/gene attribution to Minggang. Human versus mouse is a
third task with ownership and precise objective still open.

Use [Minggang's focused plan](docs/2026-09-14-SPACE-PREDICTION-HEAD-PLAN.md):

1. Specify a supported flight/control case and independent study/biological-unit splits.
2. Plan expression/PCA comparators and a simple head on frozen BridgeRNA embeddings.
3. Compare last-block tuning or LoRA only if the baseline and data warrant it.
4. Validate input-gene contributions and associated pathways across independent samples.
5. Calibrate probabilities when supported; add stress-specific heads only with explicit labels.

The initial head baseline can be developed independently of Brain's correction work;
compare original/corrected representations later under an identical evaluation protocol.
The [earlier combined pipeline](docs/2026-09-14-FINETUNING-PIPELINE.md) is technical
background, not the current task assignment or authorization to execute.
