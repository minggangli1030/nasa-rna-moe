...
The next modeling question is **technical batch suppression with biological preservation**.
Use the [PI notes and first fine-tuning proposal](docs/2026-09-14-PI-BATCH-FINETUNING.md)
for the current direction: audit poly(A)/ribodepleted preparation labels and RR1/RR3
animal links; compare frozen layers before selecting a small LoRA/gradient-reversal
pilot. Mission identity includes biology and is not automatically a removable batch.
The initial [metadata inventory](artifacts/batch_metadata_audit_2026-09-14/summary.json)
is complete. No fine-tuning run has started; the screenshot's exact cohort and
preparation labels still require verification.
