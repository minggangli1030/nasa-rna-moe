...
The [prediction-head diagnosis and remedies](artifacts/space_head_diagnostics_2026-09-14/REPORT.md)
are complete. Labels and original GPU embeddings were verified. Study-level score shifts
and reversed flight/ground ordering explain the failed cross-flight heads; consistent
FPKM preprocessing reduces the shift but does not restore transfer. Sixteen diagnostic
configurations retain the failure.

A target-specific frozen BridgeRNA head trained on six labeled chips from one pool
achieves AUROC 1.000 and balanced accuracy 0.833–1.000 on the other pool across four
within-flight tests. This is a supported exploratory adaptation case, not unseen-flight
transfer; two-label training is unstable. [Diagnostic figure](artifacts/space_head_diagnostics_2026-09-14/diagnosis_and_remedies.pdf).
The original [pilot](artifacts/space_head_pilot_2026-09-14/REPORT.md) remains intact.
...
(unassigned). No encoder tuning or BridgeRNA gene attribution has run. Nothing remains
running. See [next steps](NEXT-STEPS.md) for the narrower attribution case and the data
needed for cross-flight generalization.
...
[spaceflight-head plan](docs/2026-09-14-SPACE-PREDICTION-HEAD-PLAN.md) supersede the
