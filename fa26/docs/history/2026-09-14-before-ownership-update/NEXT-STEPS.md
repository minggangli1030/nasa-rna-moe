...
Follow the [two-stage fine-tuning pipeline](docs/2026-09-14-FINETUNING-PIPELINE.md).
The [linked benchmark audit](artifacts/bridge_benchmark_audit_2026-09-14/REVIEW.md)
identifies controlled public resources and existing results; this is no longer blocked
on receiving the mentor's additional dataset. Training has not started.

1. Prepare and verify Chen's 40 paired donors, Zhao's small external challenge and the
   author's exact RR1/RR3 mappings (17 animals, 34 profiles). Preserve input-contract
   and same-animal grouping; technical libraries are not independent biological N.
2. Reproduce expression/PCA, frozen BridgeRNA and frozen-embedding head-only baselines.
   Reuse the author's layer/readout findings before adding a limited layer probe.
3. Compare identical small LoRA updates with/without a preparation adversary, preserving
   donor variation and biological utility. Consider last-block tuning only if warranted.
   Select on grouped development data; visual batch mixing alone is insufficient.
4. Train a regularized flight head on original/adapted frozen embeddings using the same
   study/animal splits. Retain the precise ground or onboard-1g control definitions.
5. Evaluate calibration and gene/pathway attributions if held-out prediction is credible.
   Add radiation/microgravity targets only with explicit exposure/control supervision;
   model confidence is not a causal fraction of stress.

Choose the needed donor count from learning curves and uncertainty. Fourier transforms
are optional and excluded from the initial pilot. The exact screenshot membership is
still unverified; the linked benchmark's documented cohorts are separate evidence.
