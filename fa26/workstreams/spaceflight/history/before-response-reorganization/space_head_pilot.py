...
def metrics(y,p,logit):
    pred=(p>=.5).astype(int);tn,fp,fn,tp=confusion_matrix(y,pred,labels=[0,1]).ravel()
    return {'balanced_accuracy':balanced_accuracy_score(y,pred),'auroc':roc_auc_score(y,logit),'average_precision':average_precision_score(y,logit),'uncalibrated_brier':brier_score_loss(y,p),'TP':int(tp),'TN':int(tn),'FP':int(fp),'FN':int(fn)}
...
def run():
    ...
    for kind,fold,tr,te in folds:
        ...
        for name,X in features.items():
            for c in ([1,.01,100] if kind=='cross_flight' else [1]):
                model=estimator(name,c).fit(X[tr],y[tr]);p=model.predict_proba(X[te])[:,1];logit=model.decision_function(X[te])
                ...
                results.append({**tag,**metrics(y[te],p,logit)})
                predictions.extend({**tag,'id':m.iloc[i].id,'true_label':int(y[i]),'score':float(v),'decision_logit':float(l),'predicted_label':int(v>=.5)} for i,v,l in zip(te,p,logit))
                if c==1:
                    ...
                    if kind=='cross_flight':
                        for omit in tr:
                            ...
                            sensitivity.append({**tag,'omitted_train_id':m.iloc[omit].id,**metrics(y[te],q,fit.decision_function(X[te])),'prediction_agreement_with_full':float(np.mean((q>=.5)==(p>=.5)))})
    ...
    for ax,data,metric,title in [(axes[0],primary,'balanced_accuracy','Cross-flight: classification'),(axes[1],primary,'auroc','Cross-flight: ranking (flight is positive)'),(axes[2],secondary,'balanced_accuracy','Within flight: held-out pool')]:
        for j,name in enumerate(MODEL_NAMES):
            values=data[data.model==name][metric].to_numpy();offset=np.linspace(-.12,.12,len(values));ax.scatter(j+offset,values,color=colors[j],s=70,zorder=3);ax.plot([j-.2,j+.2],[values.mean()]*2,color=colors[j],lw=2)
        ax.axhline(.5,color='#555',linestyle='--',lw=1);ax.set_ylim(-.05,1.05);ax.set_xticks(range(5),[labels[x] for x in MODEL_NAMES],rotation=25,ha='right');ax.set_ylabel('AUROC from decision logits' if metric=='auroc' else 'Balanced accuracy, threshold 0.5');ax.set_title(title,fontsize=11);ax.grid(axis='y',alpha=.2)
