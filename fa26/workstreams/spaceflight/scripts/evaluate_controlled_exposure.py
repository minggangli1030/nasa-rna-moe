...
def fit(name,x,y):
    parts=[('variance',VarianceThreshold(threshold=1e-12)),('scale',StandardScaler())]
    ...
    assert np.allclose(model.named_steps['scale'].mean_,x[:,model.named_steps['variance'].get_support()].mean(0))
    return model
...
def run(root,out):
    ...
    dump(out/'verification.json',{'n_new_profiles':len(m),'n_train_IMR90':38,'input_hash_and_ID_alignment':True,'finite_inputs_embeddings_predictions':True,'whole_file_holdout_without_overlap':True,'scalers_and_PCA_training_only':True,'variance_filter_training_only':True,'minimum_training_variance':1e-12,'all_heads_converged':True,'fixed_zero_logit_threshold':True,'new_encoder_training':False,'metrics_rows':len(res),'specificity_rows':len(sp)})
