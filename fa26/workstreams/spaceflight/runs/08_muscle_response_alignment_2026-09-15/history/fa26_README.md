...
## Latest completed step — radiation robustness audit (September 15)

The [130-profile audit](workstreams/spaceflight/runs/07_radiation_robustness_audit_2026-09-15/REPORT.md)
verified treatment labels and compared common-count processing, matched-control features
and stress-negative training. A narrow positive result emerged: the frozen BRIDGE head
classified all six 2Gy/24h IMR90 cultures correctly, including after removing each entire
training expression file. At 6h, ranking is stable but the threshold is less reliable.
Chemical-stress false positives and cross-cell transfer failures remain; this is not yet
a radiation-specific explanation of flight. The new study is head-held-out but present
in the encoder pretraining catalog. [Figure](workstreams/spaceflight/runs/07_radiation_robustness_audit_2026-09-15/robustness_audit.pdf).
No job remains running. See [current status](workstreams/spaceflight/STATUS.md) and
[next validation decision](workstreams/spaceflight/PLAN.md).
