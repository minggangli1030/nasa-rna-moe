#!/usr/bin/env bash
# Strict smoke/full launcher for the coefficient-supervised Stage 2 repair.
set -euo pipefail
...
PROTOCOL="${PROTOCOL:-artifacts/stage2_organ_expert_mechanism/aligned_program_repair_protocol.json}"
...
"$PYTHON_BIN" -m py_compile \
  ...
  core/train_stage2_aligned_program_repair.py \
  evaluation/evaluate_stage2_aligned_program_repair.py
"$PYTHON_BIN" -c \
  'import json,sys; p=json.load(open(sys.argv[1])); assert p["status"]=="frozen_before_repair_training"; assert p["firewalls"]["archs4_access"] is False; assert p["firewalls"]["best_seed_selection"] is False; assert p["inputs"]["seeds"]==[17,42,101]' \
  "$PROTOCOL"
"$PYTHON_BIN" -c \
...
for seed in $RUN_SEEDS; do
  ...
  "$PYTHON_BIN" core/train_stage2_aligned_program_repair.py \
    --protocol "$PROTOCOL" \
...
if [[ "$RUN_MODE" == "full" ]]; then
  ...
  "$PYTHON_BIN" evaluation/evaluate_stage2_aligned_program_repair.py \
    --protocol "$PROTOCOL" \
    --expected-protocol-sha256 "$EXPECTED_PROTOCOL_SHA256" \
