#!/usr/bin/env bash
# Resolve the amended external shortlist with the pinned liver reserve metadata.
set -euo pipefail
...
EXPERIMENT_ROOT="${EXPERIMENT_ROOT:?set a new persistent sample-review-v2 result root}"
...
AMENDMENT="${AMENDMENT:-artifacts/stage1_k4_external_scout/sample_review_amendment_v2.json}"
...
EXPECTED_AMENDMENT_SHA256=6cd1451da17e2d5877de6617e22e31ff3e1c6e97877cb8890407038a828c75a8
...
EXPECTED_ROOT="/media/volume/moe-reboot/results/stage1_k4_external_sample_review_v2_${CODE_COMMIT:0:7}"
[[ "$EXPERIMENT_ROOT" == "$EXPECTED_ROOT" ]] || {
  echo "ERROR: EXPERIMENT_ROOT must equal $EXPECTED_ROOT" >&2
  exit 1
}
[[ ! -e "$EXPERIMENT_ROOT" ]] || {
  echo "ERROR: sample-review-v2 result root already exists" >&2
  exit 1
}
...
[[ "$(sha256sum "$AMENDMENT" | awk '{print $1}')" == "$EXPECTED_AMENDMENT_SHA256" ]] || {
  echo "ERROR: shortlist-amendment hash mismatch" >&2
  exit 1
}
...
[[ "$(sha256sum "$GEO_RESERVE_REVIEW" | awk '{print $1}')" == "$EXPECTED_GEO_RESERVE_REVIEW_SHA256" ]] || {
  echo "ERROR: supplemental GEO reserve-review hash mismatch" >&2
  exit 1
}
...
STATUS="$EXPERIMENT_ROOT/SAMPLE_REVIEW_V2_STATUS"
printf 'RUNNING phase=resolve_amended_provisional_selectors %s\n' \
...
touch "$EXPERIMENT_ROOT/PROVISIONAL_SAMPLE_SHEET_V2_COMPLETE"
...
(
  ...
  find . -type f \
    ...
    ! -name SAMPLE_REVIEW_V2_STATUS \
    ! -name METADATA_SAMPLE_REVIEW_V2_COMPLETE \
    -print0 \
    | sort -z \
...
printf 'COMPLETE %s\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" > "$STATUS"
touch "$EXPERIMENT_ROOT/READY_FOR_MANUAL_SAMPLE_REVIEW_V2"
touch "$EXPERIMENT_ROOT/METADATA_SAMPLE_REVIEW_V2_COMPLETE"
